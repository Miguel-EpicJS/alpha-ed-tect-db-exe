...
export default function Validate() {
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:4000/post/show-all-posts").then(res => setPosts(res.data));
        console.log(posts);
    ...
    const validatePost = (post) => {
        axios.put(`http://127.0.0.1:4000/post/validate-post/${post.id}`, {
    ...
    const renderCards = () => {
        return posts.map(post => {
            ...
            if (post.validated === false) {
                return ( 
                    <div className="card shadow-1" onClick={(el) => { cardClick(el) }} key={post.id}> 
                        Title: {post.title} <br/> 
                        <div className="open-show">
                            <button className="validateSubmit" onClick={() => validatePost(post)}>Validate</button>
                        </div>
                    </div> 
                );   
            }else{
